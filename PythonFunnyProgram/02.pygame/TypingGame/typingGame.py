...
class TextArea():
    ...
    def update(self):
        ...
        cur = time.time()
        if cur - self.lastupdate > 0.5:
            self.lastupdate = cur
            self.showcursor = not self.showcursor

        if len(self.blocks) > 0 and self.showcursor: #显示光标
            pygame.draw.line(self.surface,
                             (0,0,0),
                             (self.blocks[nextIdx].loc[0], self.blocks[nextIdx].loc[1] + self.cHeight * 2),
                             (self.blocks[nextIdx].loc[0] + self.cWidth, self.blocks[nextIdx].loc[1] +  + self.cHeight * 2),
                             3)
