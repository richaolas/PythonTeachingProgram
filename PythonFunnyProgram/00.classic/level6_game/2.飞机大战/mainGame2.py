...
class Config():
    ...
    def loadconfig(self):
        self.pictures_size = {}
        f = open('resources/image/shoot.pack').read()
        lines = f.split("\n")
        for i in range(5, len(lines) - 1, 7):
            temp = []
            for j in range(2, 4):
                temp.append(lines[i + j])
            self.pictures_size[lines[i]] = temp
        for i in self.pictures_size.keys():
            xywh = ''
            for j in range(len(self.pictures_size[i])):
                xywh += str(str(self.pictures_size[i][j]).split(": ")[1].split(', ')) + ","
            xywh = xywh[2:-3].replace('],[', ',').replace('\'', '').replace(' ', '').split(',')
            xywh = [int(i) for i in xywh]
            self.pictures_size[i] = xywh
        return self.pictures_size
...
class TextureSpliter(object):
    def __init__(self, name):
        ...
        for idx in range(5, line_num, 7):
            ...
            size = list(map(int, lines[idx + 2].strip().split(':')[1].split(',')))
            # pygame.Rect(xy[0],y[1],s)
            pygame.Rect(xy, size)
            self.images[name] = self.image.subsurface(pygame.Rect(267, 347, 57, 43))
...
if __name__ == "__main__":
    ...
    # f = open('resources/image/shoot.pack')
    # lines = f.readlines()
    # line_num = len(lines)
    # image_map = {}
    # for idx in range(5, line_num, 7):
    ...
    #
    #     r = pygame.Rect(xy, size)
    #     print(r)
    # print(lines[line:line+7])

    t = TextureSpliter('shoot')
